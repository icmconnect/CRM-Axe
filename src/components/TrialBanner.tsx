...
import { Clock, Sparkles, AlertTriangle, CheckCircle2, CreditCard, ShieldCheck } from 'lucide-react';
...
export default function TrialBanner() {
  ...
  if (casa.status === 'ativo') {
    ...
    return (
      <div className="w-full bg-[#ECFDF5] border-b border-[#A7F3D0] text-[#065F46] px-4 py-2.5 sm:px-6 flex items-center justify-between text-[11px] sm:text-xs font-semibold shadow-sm">
        <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="text-[10px] bg-white border border-[#A7F3D0] hover:bg-[#D1FAE5] text-[#047857] px-3 py-1 rounded-full font-bold shadow-sm transition-all"
        >
        ...
        >
          <div className="text-center mb-4 max-w-lg mx-auto">
            <h3 className="text-base font-bold text-slate-800">Alterar Plano de Assinatura</h3>
            <p className="text-xs text-slate-500 mt-1">Sua casa está ativa no plano <strong className="text-emerald-700">{planoNome}</strong>. Se desejar, escolha um plano abaixo para alterar seu pacote.</p>
          </div>
